...
public class Monster : MonoBehaviour
{
    ...
    [Header("사운드")] public AudioClip HurtSound;
    ...
    public void TakeDamageFromPlayer(float atk)
    {
        ...
        Hp -= damage;
            GameManager.Instance.SoundMgr.SoundPlay("sfx","피격음",HurtSound);
        //StartCoroutine(FlashCoroutine());
    ...
    void AttackPlayer()
    {
        ...
        if (data.MonsterType == MonsterType.원거리)
        {
            ...
            GameManager.Instance.SoundMgr.SoundPlay("sfx","원거리 공격",hitSound);
            ShootMissile();
        }
        else//근거리일때
        {
